module OrdersHelper
  ...
  def card_code_field
    if Rails.env.development?
      field = text_field_tag :card_code, nil,:class => "valid payment-form form-control", :name => "card_code_stripe", :placeholder => "3 or 4 digits", :data => {:stripe => 'cvc' }, :tabindex => 2, :value => "4242"
    else
      field = text_field_tag :card_code, nil,:class => "valid payment-form form-control", :name => "card_code_stripe", :placeholder => "3 or 4 digits", :data => {:stripe => 'cvc' }, :tabindex => 2
    end
      field.html_safe
  end
end
